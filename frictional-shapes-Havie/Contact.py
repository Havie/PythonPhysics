...
class Contact:
    ...
    def resolve(self):
        ...
        if(self.overlap > 0):
            ...
            #formerly vin
            veloInitialNormal = veloInitial * self.normal        #Vector2.dot(vi, self.normal)
            if(veloInitialNormal < 0): #if already moving away, dont reverse the velos to come back together 
                restitution = self.kwargs["restitution"]  #dictonary look up k,v
                if "friction" in self.kwargs:
                    friction = self.kwargs["friction"] #need to pass in w kwargs now 
                else:
                    friction = 0
                ...
                Jn = -(1 + restitution) * newMForRotation * veloInitialNormal 
                J = Jn * self.normal

                #Mnn #InverseMass normal normal    s_letter=displacement of CoM of obj1/s , n 
                # (s_a dot n)^2 
                InertiaObj1 = obj1.momI
                InertiaObj2 = obj2.momI
                displacement1DotNormal = pygame.Vector2.dot(displacementObj1, self.normal)
                displacement2DotNormal = pygame.Vector2.dot(displacementObj2, self.normal)

                Mnn = (1 / obj1.mass + 1/obj2.mass) + ((displacement1DotNormal * displacement1DotNormal) / InertiaObj1) + ((displacement2DotNormal* displacement2DotNormal) /InertiaObj2)
                #Mtt #Tangential 
                tangental = pygame.Vector2(-self.normal.y, self.normal.x)
                displacement1DotTangental = pygame.Vector2.dot(displacementObj1, tangental)
                displacement2DotTangental = pygame.Vector2.dot(displacementObj2, tangental)
                Mtt = (1 / obj1.mass + 1/obj2.mass) + ( (displacement1DotTangental*displacement1DotTangental) / InertiaObj1) + ((displacement2DotTangental*displacement2DotTangental) /InertiaObj2)
                #Mnt
                Mnt = ((displacement1DotNormal *displacement1DotTangental ) / InertiaObj1) + ((displacement2DotNormal *displacement2DotTangental ) / InertiaObj2)
                #  Q = madeUpLetter means nothing, Q is somekind of scalar coefficient 
                #Qn
                mew = friction # goddddddddd
                Qn= Mtt + mew * Mnt 
                #Qt
                Qt= -(Mnt + mew * Mnn) 
                #default sticking to true, then figure out if were wrong
                sticking = True;
                if(Qt >= 0):
                    sticking =False;
                else:
                    #find vns 
                    Vti = veloInitial * tangental
                    if(Vti < 0):
                        tangental= -tangental;
                        Vti = -Vti;
                    Vns = veloInitialNormal  - ((Qn/Qt) * Vti)
                    Vnf = -restitution * veloInitialNormal
                    if(Vns > Vnf):
                        sticking = False;
                
                if(sticking):
                    mewPrime = min(mew, Mnt/Mnn) #minimum in python
                    Qnp =  Mtt + (mewPrime * Mnt )
                    Qtp = -(Mnt + mewPrime * Mnn) 
                    #find new VNF
                    VnsSquared = Vns*Vns
                    VniSquared = veloInitialNormal*veloInitialNormal
                    noIdea =  (Qnp/Qn)
                    if(Vns >= 0):
                        #case 1
                        Vnf = math.sqrt((friction * friction * noIdea* VniSquared) + ((1-noIdea) * VnsSquared)) ;
                    else:
                        #case 2
                        Vnf = friction * math.sqrt(VnsSquared + (noIdea * (VniSquared - VnsSquared) ));

                Vtf = Vti + Qt/Qn * (Vnf - veloInitialNormal)
                deltaVn = Vnf - veloInitial
                deltaVt = Vtf - 0
                ...
                # newJ #Impulse = Jn *n + Jt*t
                J = pygame.math.Vector2.dot(Jn, self.normal) + pygame.math.Vector2.dot(Jt,tangental);
